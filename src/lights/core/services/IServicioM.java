...
public abstract class IServicioM<T> {
	...
	public static final String ERROR_UNKNOWN = "Error Code: ???-Error Desconocido (Servicios Web). "
			+ CONTACT;
	...
	public static final String ERROR_3 = "Error Code: 003-Ha ocurrido un error al construir el error. "
			+ CONTACT;
	...
	public String pathConsultarUno(@PathParam("idSesion") Integer idSesion,
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public String pathConsultarTodos(@PathParam("idSesion") Integer idSesion,
			@PathParam("accessToken") String accessToken,
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public String pathConsultarPorCriterios(
		...
		try {
			...
			if (validarSesion(idSesion, accessToken)) {
				return consultarPorCriterios(idSesion, typeQuery, mapaFiltros,
						orderBy);
			}
		} catch (Exception e) {
			return buildAnswerError(e);
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public String pathConsultarPorCriteriosPaginacion(
			@PathParam("idSesion") Integer idSesion,
			@PathParam("accessToken") String accessToken,
		...
		try {
			if (validarSesion(idSesion, accessToken)) {
				...
				return consultarPaginacion(idSesion, count, page,
						buildSearchFromMap(typeQuery, mapaFiltros), orderBy);
			}
		} catch (Exception e) {
			return buildAnswerError(e);
		}
	...
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public String pathCount(@PathParam("idSesion") Integer idSesion,
			@PathParam("accessToken") String accessToken) {
		try {
			if (validarSesion(idSesion, accessToken)) {
				return contar(idSesion, new Search());
			}
		} catch (Exception e) {
			return buildAnswerError(e);
		}
	...
	@GET
	@Path("/count/criteria/{idSesion}/{accessToken}")
	@Produces(MediaType.APPLICATION_JSON + ";charset=utf-8")
	public String pathCountPorCriterios(
			@PathParam("idSesion") Integer idSesion,
			@PathParam("accessToken") String accessToken, @Context UriInfo info) {
		MultivaluedMap<String, String> multivaluesMapFilters = info
				.getQueryParameters();
		...
		try {
			if (validarSesion(idSesion, accessToken)) {
				...
				return contar(idSesion,
						buildSearchFromMap(typeQuery, mapaFiltros));
			}
		} catch (Exception e) {
			return buildAnswerError(e);
		}
	...
	public String pathIncluir(String data) {
		...
		IPayloadRequest<T> request = (IPayloadRequest<T>) gson.fromJson(data,
				getClassPayloadRequest());
	...
	public String consultarTodos(Integer idSesion, String orderBy)
			throws Exception {
		List<T> objects = null;
		...
		if (orderBy.equals("")) {
			...
			search.addSort(getSortFromStringPath(orderBy));// TODO VALIDAR
															// PROPIEDAD VALIDA
	...
	public String consultarPorCriterios(Integer idSesion, TypeQuery typeQuery,
			Map<String, Object> mapaFiltros, String orderBy) throws Exception {
		List<T> objects = null;
	...
	public String consultarPaginacion(Integer idSesion,
	...
	public String eliminar(Integer idSesion, Integer id) throws Exception {
		auditar(idSesion, getTable(), AccionEnum.ELIMINAR.ordinal(), id, "");
	...
	public String contar(Integer idSesion, Search search) throws Exception {
		auditar(idSesion, getTable(), AccionEnum.CONSULTAR.ordinal(), 0, "");

		Integer count = getDAO().count(search);
	...
	public String incluir(Integer idSesion, T object) throws Exception {
		...
		auditar(idSesion, getTable(), AccionEnum.INCLUIR.ordinal(),
				getMetadataUtil().getId(o), datos);

		return buildAnswerSuccess(SUCCESS_4, mapa);
	};

	// public String incluirTodos(Integer idSesion, List<T> objects) throws
	// Exception {
	// Map<String, Object> mapa = new HashMap<String, Object>();
	//
	// int i = 1;
	...
	//
	// auditar(idSesion, getTable() , AccionEnum.INCLUIR.ordinal(),
	// getPath().substring(1) + ".INCLUIR", getMetadataUtil().getId(o), datos);
	// }
	// return buildAnswerSuccess(SUCCESS_4, mapa);
	// };
	...
	public String modificar(Integer idSesion, T object) throws Exception {
		...
		auditar(idSesion, getTable(), AccionEnum.MODIFICAR.ordinal(),
				getMetadataUtil().getId(o), datos);
	...
	public Class<?> getClassParameterizedType() throws Exception {
		...
		Type tArgument = ((ParameterizedType) superClass)
	...
	public Search buildSearchFromMap(TypeQuery typeQuery,
			Map<String, Object> mapaFiltros) throws Exception {
		Search s = new Search();
		...
		for (String key : mapaFiltros.keySet()) {
			if (mapaFiltros.get(key).getClass().equals(String.class)) {
				...
				switch (typeQuery) {
				case ILIKE:
					s.addFilterILike(key, "%" + mapaFiltros.get(key) + "%");
					break;
				case IEQUAL:
					s.addFilterIEqual(key, mapaFiltros.get(key));
					break;
				case EQUAL:
	...
	public Map<String, Object> buildMapFromMultiValuesMap(
			MultivaluedMap<String, String> multivaluesMapFilters)
			throws Exception {
		...
		for (String key : multivaluesMapFilters.keySet()) {
			...
			Class<?> clase = metadataUtil.getJavaClass(
					getClassParameterizedType(), key);

			if (clase == null) {
				throw new Exception(String.format(ERROR_7, key));
			}
			mapFilters.put(key,
					castTo(clase, multivaluesMapFilters.get(key).get(0)));
		}
	...
	private String buildAnswerGeneral(String typeMessage, boolean isOk,
			T object, String mensaje) throws Exception {
		return buildAnswerGeneral(typeMessage, isOk, object, mensaje,
				new HashMap<String, Object>());
	}

	private String buildAnswerGeneral(String typeMessage, boolean isOk,
			T object, String mensaje, Map<String, Object> datosExtras)
			throws Exception {
		...
		payloadResponse.setInformacion(IPayloadResponse.MENSAJE, typeMessage
				+ ":" + mensaje);
	...
	private String buildAnswerGeneral(String typeMessage, boolean isOk,
			List<T> objects, String mensaje) throws Exception {
		return buildAnswerGeneral(typeMessage, isOk, objects, mensaje,
				new HashMap<String, Object>());
	}

	private String buildAnswerGeneral(String typeMessage, boolean isOk,
			List<T> objects, String mensaje, Map<String, Object> datosExtras)
			throws Exception {
		IPayloadResponse<T> payloadResponse = getPayload();
	...
	private String buildAnswerGeneral(String typeMessage, boolean isOk,
		...
		return buildAnswerGeneral(typeMessage, isOk, mensaje,
				new HashMap<String, Object>());
	}

	private String buildAnswerGeneral(String typeMessage, boolean isOk,
			String mensaje, Map<String, Object> datosExtras) throws Exception {
		IPayloadResponse<T> payloadResponse = getPayload();
	...
	public IPayloadResponse<T> getPayload() throws Exception {
		String className = getClassParameterizedType().getName().replace(
				".dto.", ".payload.response.Payload")
				+ "Response";
	...
	public String buildAnswerWarning(List<T> objects, String mensaje)
			throws Exception {
	...
	public String buildAnswerSuccess(List<T> objects, String mensaje)
			throws Exception {
		return buildAnswerGeneral(SUCCESS_MESSAGE, true, objects, mensaje,
				new HashMap<String, Object>());
	}
	...
	public String buildAnswerSuccess(String mensaje) throws Exception {
		return buildAnswerGeneral(SUCCESS_MESSAGE, true, mensaje,
				new HashMap<String, Object>());
	}

	public String buildAnswerSuccess(String mensaje, Map<String, Object> map)
			throws Exception {
		return buildAnswerGeneral(SUCCESS_MESSAGE, true, mensaje, map);
	}

	public String buildAnswerInformation(T object, String mensaje)
			throws Exception {
		return buildAnswerGeneral(INFORMATION_MESSAGE, true, object, mensaje);
	}

	public String buildAnswerInformation(List<T> objects, String mensaje)
			throws Exception {
		return buildAnswerInformation(objects, mensaje,
				new HashMap<String, Object>());
	}

	public String buildAnswerInformation(List<T> objects, String mensaje,
			Map<String, Object> map) throws Exception {
		return buildAnswerGeneral(INFORMATION_MESSAGE, true, objects, mensaje,
				map);
	}
	...
	public String buildAnswerError(Exception e) {
		return buildAnswerError(e, new HashMap<String, Object>());
	}

	public String buildAnswerError(Exception e, Map<String, Object> parametros) {
		try {
			...
			return "{\"informacion\":{\"isOK\":false,\"mensaje\":\"E:"
					+ ERROR_3 + "\"}}";
		}
	}
	...
	public Sort getSortFromStringPath(String orderby) {
		orderby = orderby.split("/")[2]; // -- '/orderby/propiety=value'

		return new Sort(orderby.split("=")[0], new Boolean(
				orderby.split("=")[1]));
	}
	...
	public abstract boolean validarSesion(Integer idSesion, String accessToken)
			throws Exception;

	public abstract void auditar(Integer idSesion, String fkTabla,
			Integer accion, Integer registroId, String datos) throws Exception;
}